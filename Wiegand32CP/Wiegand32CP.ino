...
const int W_D1 = 2; // Wiegand data 0 line is assigned to Arduino pin D2
const int W_D0 = 3; //   "       "  1  "            "            "    D3
...
void outwieg32(uint32_t u32); // output a Wiegand-32 code
...
// outputs a 32 bit Wiegand code
// u32 is actually the 32-bit numeric code
void outwieg32(uint32_t u32)
{
  uint32_t tmp = u32;
...
{
  char msg[64];
  unsigned long l = strtoul(str, NULL, 10);
  if (l < 0 || l > 0xFFFFFFFF)
  {
    Serial.write("ERROR\n");
    return;
  }
  sprintf(msg, "OK: %lu (0x%08lX)\n", l, l);
  Serial.write(msg);
  outwieg32((unsigned long) l);
  delay(1000);
}